...
結論から言えば、[こちら](https://github.com/msakuta/rusty-parser/blob/master/parser/examples/dyn_prec.rs)に示すコードのように書けば可能です。

## 演算子の定義のデータ構造への変更

まずは `OpCode` の定義は次のように変わります。ハードコードされた列挙型の代わりに文字列でコードを持ち、優先順位と結合性をフィールドに持ちます。
