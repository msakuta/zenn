...
tower-lsp-boilerplate では `./dist` が出力先となっているため、 `tsc -b` でコンパイルすると `./client/node_packages` にインストールされているライブラリが参照できなくなり、起動できませんでした。これを動作させるには、ライブラリも含めてバンドルする `npm run compile` (それによって `node esbuild.js --production` を実行する)か、出力先を `./client` に変更する必要があります。これは恐らく tower-lsp-boilerplate の作者による間違いです。この拡張はほとんどクライアントサイドのコードがないのでバンドルしても良いのですが、私は後者の方法を選びました。
